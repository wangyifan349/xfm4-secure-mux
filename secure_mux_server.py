...
def server_handshake(sock: socket.socket) -> Tuple[bytes, bytes, str]:
    ...
    if len(hello) != len(PROTOCOL_MAGIC) + PUBLIC_KEY_SIZE:
        raise ProtocolError("Invalid client handshake length")
...
def create_listener(family: int, host: str, port: int) -> socket.socket:
...
def create_server_listeners(host: Optional[str], port: int, console: Console) -> List[socket.socket]:
    listeners: List[socket.socket] = []
    specifications: List[Tuple[int, str]] = []

    if host is None or host.strip() in {"", "*"}:
        specifications.append((socket.AF_INET, "0.0.0.0"))
        specifications.append((socket.AF_INET6, "::"))
    else:
        normalized_host = strip_ipv6_brackets(host)
        try:
            ...
            raise ValueError(f"Unable to resolve listen address {host}: {exc}") from exc

        seen: set[Tuple[int, str]] = set()
        for family, socket_type, protocol, canonical_name, socket_address in address_info:
            del socket_type, protocol, canonical_name
            if family not in {socket.AF_INET, socket.AF_INET6}:
                continue
            specification = (family, str(socket_address[0]))
            if specification in seen:
                continue
            seen.add(specification)
            specifications.append(specification)

    for family, listen_host in specifications:
        try:
            listener = create_listener(family, listen_host, port)
        except OSError as exc:
            endpoint = format_endpoint((listen_host, port))
            console.write(f"Unable to listen on {endpoint}: {exc}")
            continue
        listeners.append(listener)
        console.write(f"Listening on {format_endpoint(listener.getsockname())}")

    if not listeners:
        raise OSError("No IPv4 or IPv6 listener could be started")
    return listeners


def close_listeners(listeners: Iterable[socket.socket]) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="XFM4 X25519 + ChaCha20-Poly1305 bidirectional multiplexed server"
    )
    parser.add_argument(
        "--host",
        default=None,
        help="Listen address. The default listens on both 0.0.0.0 and ::",
    )
    ...
    try:
        ...
        listeners = create_server_listeners(args.host, args.port, console)
        for listener in listeners:
            selector.register(listener, selectors.EVENT_READ)
        console.write(
            "The server handles one peer at a time and accepts the next peer after disconnect."
        )
        ...
        while True:
            ...
            for key, event_mask in events:
                ...
                accepted = True
                peer_ip = normalize_ip_address(address)
                peer_endpoint = format_endpoint(address)
                console.write(f"Accepted connection from {peer_endpoint}")
                try:
                    client_sock.settimeout(20)
                    send_key, receive_key, fingerprint = server_handshake(client_sock)
                    client_sock.settimeout(None)
                    console.write(
                        f"Encrypted session established with {peer_ip}. "
                    ...
                    console.write(f"Connection handler failed for {peer_endpoint}: {exc}")
                    try:
                        client_sock.close()
                    except OSError:
                        pass
        ...
        console.write(f"Server startup failed: {exc}")
    finally:
        selector.close()
